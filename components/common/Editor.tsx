    ...
    return (
        <div className="mx-auto bg-[#0f172a]">
            <div className="grid grid-cols-[auto_1fr] text-[clamp(0.75rem,1.2vw,1.1rem)] leading-relaxed">
                <div className="bg-[#0b1220] text-slate-500 px-3 py-4 text-right select-none sm:px-4">
                    {lines.map((_, i) => (
                        <div key={i}>{i + 1}</div>
                    ))}
                </div>
                <pre className="px-3 py-4 whitespace-pre-wrap wrap-break-words text-slate-300 sm:px-6">
                    <code>
                        {lines.map((text, i) => (
